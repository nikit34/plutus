    ...
    <div className="space-y-8 max-w-2xl">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-6 rounded-2xl bg-bg-card border border-border">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-5 mb-5">
            <div className="w-16 h-16 rounded-2xl bg-gold/20 flex items-center justify-center text-xl font-semibold text-gold">{creator.name.split(' ').map((n) => n[0]).join('')}</div>
            <button className="px-4 py-2 rounded-xl bg-bg-elevated border border-border text-sm hover:bg-bg-hover transition-colors">Upload photo</button>
          </div>
          <div>
          ...
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-bg-input border border-border text-text-primary focus:border-gold/30 transition-colors text-[15px]" />
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} className="p-6 rounded-2xl bg-bg-card border border-border">
        ...
        <div className="space-y-3">
          <input type="url" defaultValue={creator.socialLink || ''} placeholder="https://youtube.com/..., instagram.com/..., tiktok.com/..." className="w-full px-4 py-3 rounded-xl bg-bg-input border border-border text-text-primary placeholder:text-text-tertiary focus:border-gold/30 transition-colors text-[15px]" />
          <input type="text" defaultValue={creator.socialLabel || ''} placeholder="Label: YouTube, Instagram, TikTok..." className="w-full px-4 py-3 rounded-xl bg-bg-input border border-border text-text-primary placeholder:text-text-tertiary focus:border-gold/30 transition-colors text-[15px]" />
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="p-6 rounded-2xl bg-bg-card border border-border">
        <div className="flex items-center gap-2 mb-5"><Bell size={16} className="text-text-secondary" /><h3 className="text-base font-semibold">Notifications</h3></div>
        <div className="space-y-3">
          {[
            { label: 'New sale', desc: 'Get notified on every purchase', default: true },
            { label: 'Weekly report', desc: 'Weekly stats summary via email', default: true },
            { label: 'AI recommendations', desc: 'Tips to increase sales', default: false },
          ].map((item) => (
      ...
      <button onClick={() => addNotification('Settings saved!')} className="px-6 py-3 rounded-xl bg-gold text-bg-primary font-semibold text-sm hover:brightness-110 transition-all">Save changes</button>
    </div>
  );
}
...
function ToggleSwitch({ defaultOn = false }) {
  ...
  return (
    <button onClick={() => setOn(!on)} className={`w-11 h-6 rounded-full transition-colors relative ${on ? 'bg-gold' : 'bg-bg-hover'}`}>
